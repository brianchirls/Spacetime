...
const
	...
	*/
	...
	publicClipMethods = [
		...
		'clone'
		//todo: get/set
		//todo: allow force load?
		//todo: additional state queries?
	];
...
function Clip(clip) {
	...
	*/
	...
	publicClipMethods.forEach((key) => {
		this[key] = clip[key].bind(clip);
	});

	eventEmitterize(this);

	/*
	todo: broadcast certain events from internal clip, but do not allow removal
	of critical listeners
	*/
}

function ClipInternal(internal, plugin, options) {
	/*
	...
	*/
	function localPlayTime(time) {
		// todo: account for looping clip
		return Math.min(time - start + from, Math.min(to, playerMethods.duration() || Infinity));
	}
	...
	function updateClipBuffered() {
		...
		if (typeof buffered === 'number' && !isNaN(buffered)) {
			if (playerMethods.readyState()) {
				buffered = Math.min(Math.max(0, buffered - from), duration);
			} else {
				buffered = 0;
			}
			...
			for (i = ranges.length - 1; i >= 0; i--) {
				...
				range.start = Math.min(Math.max(0, buffered.start(i) - from), duration);
				range.end = Math.min(Math.max(0, buffered.end(i) - from), duration);
		...
		// if clip extends beyond media duration, fill in buffered to end
		max = Math.min(to, playerMethods.duration() || Infinity) - from;
		if (self.buffered.includes(max)) {
			self.buffered.add(duration - max, duration);
		}
	}
	...
	function reset(player) {
		...
		*/
		...
		if (!isNaN(playerMethods.duration()) && playerMethods.readyState()) {
			self.loadMetadata(self.metadata);
		}
	}
	...
	*/
	...
	this.play = function () {
		if (playerMethods.currentTime() < playerMethods.duration()) {
			playerMethods.play();
		}
	};
